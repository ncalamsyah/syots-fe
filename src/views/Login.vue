        ...
        <div class="w-full px-8 md:px-32 lg:px-24">
          ...
          >
            ...
            <p class="text-sm font-normal text-gray-600 mb-8">
              Please Signin and Write Your Stories!
            </p>
            ...
            <RouterLink :to="{ name: 'Login Admin' }">
              <button
                type="submit"
                class="block w-full bg-primary-variant mt-5 py-2 rounded-2xl hover:bg-indigo-800 hover:-translate-y-1 transition-all duration-500 text-white font-semibold mb-2"
              >
                Login as Admin
              </button>
            </RouterLink>
  ...
  methods: {
    async handleSubmit() {
      ...
      if (response.status == "200") {
        ...
        this.$router.push(`/profile/${response.data.data.username}`);
      } else {
      ...
      const responseThought = await axios.get(
        `/thought/${response.data.data.id}`
      );
      console.log(responseThought);
      localStorage.setItem("thoughts", responseThought.data.data);
    },
  },
};
</script>
